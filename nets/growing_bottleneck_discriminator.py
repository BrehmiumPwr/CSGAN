...
from .blocks import residual_block, bottleneck


class GrowingBottleneckDiscriminator(GrowingDiscriminator):
    ...
    def block(self, x, output_dim, s=1, name="block"):
        return residual_block(x, dim=output_dim, act=self.act, norm=self.norm, sn=self.sn, ks=3, s=s, type=bottleneck,
                              name=name)
