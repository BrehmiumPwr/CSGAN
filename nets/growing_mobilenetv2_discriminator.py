...
from .blocks import residual_block, inverse_bottleneck


class GrowingMobilenetV2Discriminator(GrowingDiscriminator):
    ...
    def block(self, x, output_dim, s=1, name="block"):
        return residual_block(x, dim=output_dim, act=self.act, norm=self.norm, sn=self.sn, ks=3, s=s,
                              type=inverse_bottleneck, name=name)
